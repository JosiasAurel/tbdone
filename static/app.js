...
function initMission() {
    ...
    targetDays =
        localStorage.getItem("targetDays") ??
        prompt("For how many days do you plan to do this");
    targetDays = parseInt(targetDays);

    progressTicks =
        localStorage.getItem("progess") ?? new Array(targetDays).fill(false);
...
function fillProgressTicks() {
    ...
    progressTicks.forEach((state, i) => {
        ...
        state ? (_tick.checked = true) : (_tick.checked = false);
        container.appendChild(_tick);
        _tick.addEventListener("click", (_e) => {
            const tickDay = new Date(
...
const updateProgressTicks = (_) =>
    localStorage.setItem("progress", progressTicks.join("-"));

function boolFromStringArray(arr) {
    return arr.map((item) => (item === "true" ? true : false));
}

// load and desrialize progress ticks
function loadProgressTicks() {
    return boolFromStringArray(localStorage.getItem("progress").split("-"));
}
